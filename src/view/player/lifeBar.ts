...
export class LifeBar extends GameObjects.Container {
  private fullColor = Phaser.Display.Color.HexStringToColor(LifeColors.full)
    .color;
  private middleColor = Phaser.Display.Color.HexStringToColor(LifeColors.middle)
    .color;
  private lowColor = Phaser.Display.Color.HexStringToColor(LifeColors.low)
    .color;
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, width: number) {
    ...
    const background = scene.add.rectangle(
      ...
      width,
      width / 10,
      Phaser.Display.Color.HexStringToColor("CBC7C7").color,
      0.7
    );
