...
export class MapManager {
  ...
  protected _onMapReady = new Subject<Map["id"]>();
  ...
  public hydratateMap(id: Map["id"], map: Partial<ProcessedMap>) {
    if (!this.processedMaps[id]) return;

    Object.keys(map).forEach((key) => {
      this.processedMaps[id][key] = map[key];
    });
    this._onMapReady.next(id);
  }
  ...
  get onMapReady(): Observable<Map["id"]> {
    return this._onMapReady;
  }
}
